...
// API Client
function CoreBackendClient({ protocol, host, port, basePath } = {}, token) {
  ...
  const axiosConfig = Object.assign({}, { baseURL }, headers);
  console.log('CoreBackendClient configured to', axiosConfig);

  return require('axios').create(axiosConfig);
}

/**
 * Initialise `serviceContext` then associate it with request object.
 */
app.use((request, response, next) => {
  const token = request.cookies && request.cookies.token;
  const apiClient = new CoreBackendClient(config('core.backend'), token);

  const localStorage = {
    set: (key, value) => response.cookie && response.cookie(key, value),
    get: key => request.cookies && request.cookies[key],
    remove: key => response.clearCookie && response.clearCookie(key),
  };
